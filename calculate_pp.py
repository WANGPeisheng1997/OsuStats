...
def calculate_pp(beatmap, c100, c50, misses, used_mods=Mods(), combo=None, score_version=1, c300=None):
    res = PPCalcResult()
    od = beatmap.od
    ar = beatmap.ar
    circles = beatmap.count_normal
    num_objects = beatmap.num_objects

    if c100 > num_objects or c50 > num_objects or misses > num_objects:
        print("Invalid accuracy number")
        return res

    if c300 is None:
        c300 = beatmap.num_objects - c100 - c50 - misses
    ...
    total_hits = c300 + c100 + c50 + misses
    if total_hits != beatmap.num_objects:
    ...
    aim_value = base_strain(beatmap.aim)
    ...
    combo_break = math.pow(combo, 0.8) / math.pow(beatmap.max_combo, 0.8)
    ...
    speed_value = base_strain(beatmap.speed)
